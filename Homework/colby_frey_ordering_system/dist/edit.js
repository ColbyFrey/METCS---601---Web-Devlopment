...
function updateOrder(id, name, quantity) {
    const orders = getOrders();
    const updatedOrders = orders.map((order) => order.id === id ? new Order(order.id, name, quantity, order.createdAt) : order);
    saveOrders(updatedOrders);
    renderEditOrders();
}
function completeOrder(id) {
    const orders = getOrders();
    const updatedOrders = orders.map((order) => order.id === id ? new Order(order.id, order.name, order.quantity, order.createdAt, true) : order);
    saveOrders(updatedOrders);
    renderEditOrders();
}
...
function setupEventListeners() {
    ...
    editTable === null || editTable === void 0 ? void 0 : editTable.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("edit-btn")) {
            ...
            const quantityInput = document.querySelector(`.edit-quantity[data-id="${id}"]`);
